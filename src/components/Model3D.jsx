...
import modelPath from '../assets/models/RayStevensonTSLikenessModel.glb'

function Model3D({ rotation, opacity, translateY, scale }) {
  ...
  useEffect(() => {
    if (clonedScene) {
      ...
      const baseScaleFactor = maxDim > 0 ? 2 / maxDim : 1
  ...
  // Appliquer l'opacité dans un useEffect pour s'assurer qu'elle est mise à jour
  useEffect(() => {
    if (clonedScene) {
      clonedScene.traverse((child) => {
        if (child.isMesh) {
